...
def detect(opt, data_path, classifier_seg3D, classifier_ce, classifier_Rot_green, classifier_Rot_red,
    ...
    for icc, data in enumerate(dataloader):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                iou3d, R_loss, T_loss = FS_Net_Test(dep3d, pc[0].numpy(), im0, Rt, Tt, classifier_seg3D, classifier_ce,
                                                    classifier_Rot_green,
                                                    classifier_Rot_red,
                                                    cate, model_size, cate_id0, num_cor=3)
                # time.sleep(0.1)
    ...
    eva = ['IoU:%4f' % (iou_aver * 100), 'IoU75:%4f' % (iou75 * 100), 'IoU50:%4f' % (iou50 * 100),
           '5d5cm:%4f' % (d5cm5 * 100),
           '10d5cm:%4f' % (d10cm5 * 100), 'R_loss:%4f' % R_aver, 'T_loss:%4f' % T_aver]
...
if __name__ == '__main__':
    ...
    # fold = '/home/charlielee/grasp_pybullet/dataset/%s/1/' % cate

    classifier_seg3D, classifier_ce, classifier_Rot_green, classifier_Rot_red, model_size, cate_id0 = load_models(cate)
    with torch.no_grad():
