...
from BSNSeg.lib.models import model_factory
from BSNSeg.configs import set_cfg_from_file
from BSNSeg.lib.logger import setup_logger
from BSNSeg.lib.get_dataloader import get_data_loader
...
class MscEvalV0(object):
    ...
    def __call__(self, net, dl, n_classes):
        ...
        for i, (imgs, label) in diter:
            ...
            N, _, H, W = label.shape
            pic = imgs[0].numpy().transpose(1, 2, 0)
            lb = label[0].numpy().squeeze()
            ...
            pd = preds[0].cpu().numpy()            
            
            classes = ['background', 'blue', 'red', 'green', 'yellow', 'purple', 'orange']
            labels = []
            class_ids = []
            masks = []
            canvas = np.zeros((n_classes, pd.shape[0], pd.shape[1]))
            for i in np.unique(pd):
                canvas[i][pd==i]=1
                if i != 0:
                    if np.sum(canvas[i]) < 300:
                        canvas[i] = 0
                    else:
                        class_ids.append(i)
                        mask = cv2.resize(canvas[i], (224, 224))
                        masks.append(mask)
            ...
            if len(class_ids) > 0:
                ...
                masks[masks < 0.9] = 0
                masks = np.array(masks)
                seg_res = {'class_ids': class_ids, 'labels': labels, 'masks': masks}
            else:
                seg_res = {'class_ids': class_ids, 'labels': labels, 'masks': masks}
...
def eval_model(cfg, net):
    ...
    # print(single_scale)
    mIOU, segment_result = single_scale(net, dl, cfg.n_cats)
    ...
    return heads, mious, segment_result
...
def evaluate(cfg, weight_pth):
    ...
    net.to(device)
    ## evaluator
    heads, mious, segment_result = eval_model(cfg, net.module)
...
def parse_args():
    ...
    parse.add_argument('--weight-path', dest='weight_pth', type=str,
                       default='BSNSeg/res/model_final.pth',)
    # parse.add_argument('--port', dest='port', type=int, default=44553,)
    parse.add_argument('--config', dest='config', type=str,
            default='BSNSeg/configs/bisenetv2_city.py',)
...
def segment():
    ...
    if not osp.exists(cfg.respth): os.makedirs(cfg.respth)
    setup_logger('{}-eval'.format(cfg.model_type), cfg.respth)
    segment_result = evaluate(cfg, args.weight_pth)
