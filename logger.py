...
class Logger():
    ...
    def __init__(self, base_directory):
        ...
        self.grasp_mask_heightmaps_directory = os.path.join(self.base_directory, 'data', 'grasp-mask-heightmaps')
        self.push_mask_heightmaps_directory = os.path.join(self.base_directory, 'data', 'push-mask-heightmaps')
        ...
        if not os.path.exists(self.grasp_mask_heightmaps_directory):
            os.makedirs(self.grasp_mask_heightmaps_directory)
        if not os.path.exists(self.push_mask_heightmaps_directory):
            os.makedirs(self.push_mask_heightmaps_directory)
    ...
    def save_heightmaps(self, iteration, depth_heightmap, target_mask_heightmap, grasp_mask_heightmap):
        depth_heightmap = np.round(depth_heightmap * 100000).astype(np.uint16) # Save depth in 1e-5 meters
        cv2.imwrite(os.path.join(self.depth_heightmaps_directory, '%06d.depth.png' % iteration), depth_heightmap)
        cv2.imwrite(os.path.join(self.target_mask_heightmaps_directory, '%06d.mask.png' % iteration), (target_mask_heightmap * 255).astype(np.uint8))
        cv2.imwrite(os.path.join(self.grasp_mask_heightmaps_directory, '%06d.grasp.png' % iteration), (grasp_mask_heightmap * 255).astype(np.uint8))  # grasp mask
        # cv2.imwrite(os.path.join(self.grasp_mask_heightmaps_directory, '%06d.push.png' % iteration), (grasp_mask_heightmap * 255).astype(np.uint8))  # push mask
