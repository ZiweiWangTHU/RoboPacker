...
class CornellDataset(GraspDatasetBase):
    ...
    def _get_crop_attrs(self, idx):
        ...
        # left = max(0, center[1] - 150)
        return center, left, top

    def _get_crop_attrs2(self, center):

        left = max(0, min(center[0] - 150, 640 - 300))
        top = max(0, min(center[1] - 150, 480 - 300))
    ...
    def get_rgb2(self, idx, center, rot=0, zoom=1.0, normalise=True):
        rgb_img = image.Image.from_file(self.rgb_files[idx])
        # center, left, top = self._get_crop_attrs2(center)
        center, left, top = self._get_crop_attrs(idx)
