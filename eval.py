...
def prep_display(dets_out, img, h, w, undo_transform=True, class_color=False, mask_alpha=0.45, fps_str=''):
    ...
    with timer.env('Postprocess'):
        ...
        # save_mask
        mask_base_path = os.getcwd()
        mask_save_path = os.path.join(mask_base_path, "save_mask")
        print("save_path", mask_save_path)
        mask = t[3].clone()

        mask = mask.cpu().numpy()
        for mask_id in range(mask.shape[0]):
            path_one = os.path.join(mask_save_path, str(mask_id)+".png")
            mask_img = Image.fromarray(np.uint8(mask[mask_id]))
            mask_img.save(path_one)
...
class APDataObject:
    ...
    def get_ap(self) -> float:
        ...
        return sum(y_range) / len(y_range), self.data_points
